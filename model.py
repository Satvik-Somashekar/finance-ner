...
import codecs
import pickle                               # CHANGED: cPickle → pickle (removed in Python 3)
...
class Model(object):
    ...
    def __init__(self, parameters=None, models_path=None, model_path=None):
        if model_path is None:
            ...
            with open(self.parameters_path, 'wb') as f:
                pickle.dump(parameters, f)  # CHANGED: cPickle → pickle
        else:
            ...
            with open(self.parameters_path, 'rb') as f:
                self.parameters = pickle.load(f)  # CHANGED: cPickle → pickle
    ...
    def save_mappings(self, id_to_word, id_to_char, id_to_tag):
        ...
        with open(self.mappings_path, 'wb') as f:
            ...
            pickle.dump(mappings, f)        # CHANGED: cPickle → pickle

    def reload_mappings(self):
        with open(self.mappings_path, 'rb') as f:
            mappings = pickle.load(f)       # CHANGED: cPickle → pickle
    ...
    def build(self, dropout, char_dim, char_lstm_dim, char_bidirect,
              word_dim, word_lstm_dim, word_bidirect, lr_method,
              pre_emb, crf, cap_dim, training=True, **kwargs):
        ...
        if word_dim:
            ...
            if pre_emb and training:
                ...
                print('Loading pretrained embeddings from %s...' % pre_emb)  # CHANGED: print()
                pretrained = {}
                emb_invalid = 0
                for i, line in enumerate(codecs.open(pre_emb, 'r', 'utf-8')):
                    ...
                    if len(line) == word_dim + 1:
                        ...
                        emb_invalid += 1
                if emb_invalid > 0:
                    print('WARNING: %i invalid lines' % emb_invalid)  # CHANGED: print()
                c_found = 0
                c_lower = 0
                c_zeros = 0
                for i in range(n_words):                               # CHANGED: xrange → range
                    ...
                    if word in pretrained:
                        ...
                        c_lower += 1
                    elif re.sub(r'\d', '0', word.lower()) in pretrained:  # CHANGED: raw string
                        new_weights[i] = pretrained[
                            re.sub(r'\d', '0', word.lower())           # CHANGED: raw string
                        ]
                        c_zeros += 1
                word_layer.embeddings.set_value(new_weights)
                print('Loaded %i pretrained embeddings.' % len(pretrained))  # CHANGED: print()
        ...
        if word_bidirect:
            final_output = T.concatenate(
                [word_for_output, word_rev_output], axis=1)
